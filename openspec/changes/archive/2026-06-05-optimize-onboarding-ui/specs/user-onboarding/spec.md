...
- Bottom actions: The bottom controls and actions area MUST maintain a stable layout height of 124px across all slides to prevent vertical shifts of page indicators and slide contents during swiping.
...
The transition between the single "Next" action button on earlier slides and the stacked wallet mode buttons on the final slide MUST be animated smoothly using `AnimatedSwitcher` without modifying the height of the bottom controls container.
